// 迁移脚本共用的指纹生成 helper（与 src/lib/dedup/fingerprint.ts 逻辑一致，纯 JS 供 node 脚本调用）
export function generateFingerprintFromText(input) {
  ...
  // 优先使用已结构化的 scenarios 字段（cases 真实字段为 scenarios 数组，含 slug）
  let scenario;
  ...
  const department = input.department || DEPT.find((d) => text.includes(d));
