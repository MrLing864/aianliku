import type { CaseStudy, ValueTier } from "@/lib/types";

// 案例价值分级（与 scripts/value-tier.mjs 保持逻辑一致——改这里务必同步改那边）。
...
const TIERS: Array<{ tier: ValueTier; min: number }> = [
  { tier: "extreme", min: 75 },
  { tier: "high", min: 60 },
  { tier: "medium", min: 40 },
  { tier: "low", min: 0 },
...
const MONEY_KEYWORDS = /营收|收入|利润|成本|收益|销售额|产值|节约|降本|增收|投资|回报|费用|金额|资金|经费|英镑|美元|人民币|欧元|日元|港元/;
...
function parseMoneyMax(text: string | undefined): number {
  if (!text) return 0;
...
function parsePercentBonus(text: string | undefined): number {
  if (!text) return 0;
  let bonus = 0;
  const re = /([+-]?\d+(?:\.\d+)?)\s*%/g;
  let m: RegExpExecArray | null;
...
function benefitFromText(text: string | undefined): number {
  if (!text) return 0;
  const t = String(text);
...
function scoreBenefit(c: CaseStudy): number {
...
function scoreBenchmark(c: CaseStudy): number {
  let score = 15;
...
function scoreReference(c: CaseStudy): number {
  let score = 10;
...
export function computeValueTier(c: CaseStudy): { tier: ValueTier; score: number } {
...
export const VALUE_TIER_ORDER: ValueTier[] = ["extreme", "high", "medium", "low"];
