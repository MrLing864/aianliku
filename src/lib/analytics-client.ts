...
export function trackPublicEvent(name: PublicEventName, caseId?: string, metadata?: { policyVersion?: string }) {
  const body = JSON.stringify({ name, caseId, path: window.location.pathname, ...metadata });
