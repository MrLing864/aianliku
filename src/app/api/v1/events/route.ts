...
function safePublicPath(value?: string) {
  ...
  if (["/admin", "/reports/", "/assessment/status/", "/api/"].some((prefix) => value.startsWith(prefix))) return undefined;
...
function anonymousVisitorKey(request: Request, dateKey: string) {
  ...
  return createHmac("sha256", salt).update(`${dateKey}|${forwarded}|${userAgent}`).digest("hex");
}

export async function POST(request: Request) {
  const rateLimit = await checkRateLimit(request, "analytics-event", 180, 60 * 60 * 1_000);
  if (!rateLimit.allowed) return NextResponse.json({ error: "rate_limited" }, { status: 429, headers: { "Retry-After": String(rateLimit.retryAfterSeconds) } });
  const parsed = eventSchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success) return NextResponse.json({ error: "invalid_event" }, { status: 400 });
  if (parsed.data.name === "qualified_case_reader" && (!parsed.data.caseId || (parsed.data.durationSeconds ?? 0) < 30 || (parsed.data.readingDepth ?? 0) < 50)) return NextResponse.json({ error: "invalid_qualified_reader" }, { status: 400 });
  if (isMongoConfigured()) {
    ...
    const dedupeKey = parsed.data.name === "qualified_case_reader" ? `${dateKey}:${visitorKey}:${parsed.data.caseId}` : undefined;
    try {
      await db.collection("analytics_events").insertOne({ id: nanoid(), ...parsed.data, path, dedupeKey, visitorKeyVersion: 1, occurredAt });
      if (parsed.data.name === "qualified_case_reader") await db.collection("cases").updateOne({ id: parsed.data.caseId, contentStatus: "published" }, { $inc: { views: 1 } });
    } catch (error) {
