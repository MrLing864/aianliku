      ...
      <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_300px] lg:gap-14">
        <div><div className="flex flex-wrap gap-2"><OutcomeBadge status={item.outcomeStatus} /><ConfidenceBadge level={item.confidence} />{item.demo && <Badge variant="secondary">演示数据</Badge>}</div><p className="mt-7 text-xs font-semibold text-primary">{item.industry?.displayName ?? "其他行业"} · {(item.scenarios ?? []).map((scene: { name: string }) => scene.name).join(" / ") || "AI应用"}</p><h1 className="mt-3 text-balance text-4xl font-semibold leading-[1.12] tracking-[-0.05em] sm:text-5xl">{item.title}</h1>        <p className="mt-6 max-w-3xl text-lg leading-8 text-muted-foreground">{item.summary}</p>
          ...
          <section className="mt-12 rounded-2xl border bg-card p-6 sm:p-8">
            ...
            {item.painPointNarrative && <p className="mt-4 text-[15px] leading-8 text-foreground/80">{item.painPointNarrative}</p>}
            {(item.painPointTags?.length ?? 0) > 0 && (
              <div className="mt-4 flex flex-wrap gap-2">
                {item.painPointTags!.map((tag) => <Badge key={tag} variant="secondary">{tag}</Badge>)}
              </div>
            )}
          </section>
        )}
        ...
        {item.testimonial && (
          <section className="mt-12 rounded-2xl border bg-card p-6 sm:p-8">
            ...
            <p className="mt-5 text-lg font-medium leading-8">“{item.testimonial.quote}”</p>
            {(item.testimonial.author || item.testimonial.authorTitle) && <p className="mt-4 text-sm text-muted-foreground">{[item.testimonial.author, item.testimonial.authorTitle].filter(Boolean).join(" · ")}</p>}
          </section>
        )}
          <section className="mt-12"><div className="flex items-center justify-between"><h2 className="text-xl font-semibold">信息来源</h2><Badge variant="outline">{item.sources.length} 个来源</Badge></div><p className="mt-2 text-sm leading-6 text-muted-foreground">页面内容为结构化改写；关键结论应可追溯到以下材料。</p>{item.sourceReport?.title && <p className="mt-2 text-xs text-muted-foreground">关联报告：{item.sourceReport.title}{item.sourceReport.publisher ? ` · ${item.sourceReport.publisher}` : ""}{item.sourceReport.year ? ` · ${item.sourceReport.year}` : ""}</p>}<div className="mt-5 space-y-3">{item.sources.map((source, index) => <Card key={source.id} className="py-0 shadow-none"><CardContent className="flex gap-4 p-5"><span className="font-mono text-xs text-muted-foreground">{String(index + 1).padStart(2, "0")}</span><div className="min-w-0 flex-1">{source.url ? <a href={source.url} target="_blank" rel="noreferrer" className="font-medium text-foreground hover:text-primary hover:underline">{source.title}</a> : <p className="font-medium">{source.title}</p>}<p className="mt-1.5 text-xs text-muted-foreground">{source.publisher}{source.publishedAt ? ` · ${source.publishedAt}` : ""}</p><p className="mt-3 text-xs leading-5 text-muted-foreground">支持：{source.supports.join("、")}</p></div>{source.url && <Button variant="outline" size="icon" asChild><a href={source.url} target="_blank" rel="noreferrer" aria-label="查看原文"><ExternalLink /></a></Button>}</CardContent></Card>)}</div></section>
        <div className="mt-10"><Button variant="ghost" size="sm" asChild><Link href="/cases"><ArrowLeft />返回案例库</Link></Button></div>
        </div>
        <aside className="lg:sticky lg:top-24 lg:self-start"><Card className="shadow-none"><CardContent className="space-y-5"><h2 className="font-semibold">实施概览</h2><Separator />{[{ icon: Building2, label: "企业规模", value: item.organization.size + (item.organization.type ? " · " + (ORG_TYPE_LABEL[item.organization.type] ?? "") : "") }, { icon: CalendarDays, label: "实施年份", value: item.implementationYear ? String(item.implementationYear) : "未披露" }, { icon: Clock3, label: "实施周期", value: item.projectDuration?.disclosed && item.projectDuration.minWeeks ? item.projectDuration.minWeeks + "–" + (item.projectDuration.maxWeeks ?? item.projectDuration.minWeeks) + " 周" : item.duration }, { icon: WalletCards, label: "投入成本", value: item.investmentRange?.disclosed ? (item.investmentRange.min != null && item.investmentRange.max != null ? item.investmentRange.min + "–" + item.investmentRange.max + " 万" + (item.investmentRange.currency === "USD" ? "美元" : "") : (item.investmentRange.narrative ?? "未披露")) : item.cost }, { icon: ShieldCheck, label: "ROI", value: item.roi }].map(({ icon: Icon, label, value }) => <div key={label} className="flex gap-3"><Icon className="mt-0.5 size-4 shrink-0 text-primary" /><div><p className="text-xs text-muted-foreground">{label}</p><p className="mt-1 text-sm font-medium leading-6">{value}</p></div></div>)}{item.implementers.length > 0 && <><Separator /><div><p className="text-xs text-muted-foreground">实施方</p><div className="mt-2 flex flex-wrap gap-1.5">{item.implementers.map((impl) => <Badge key={impl.name} variant="secondary">{impl.name}{impl.role ? " · " + impl.role : ""}</Badge>)}</div></div></>}{item.modelStack && item.modelStack.length > 0 && <><Separator /><div><p className="text-xs text-muted-foreground">适用大模型</p><div className="mt-2 flex flex-wrap gap-1.5">{item.modelStack.map((m) => <Badge key={m} variant="outline">{m}</Badge>)}</div></div></>}{item.techPath && item.techPath.length > 0 && <><Separator /><div><p className="text-xs text-muted-foreground">技术路径</p><div className="mt-2 flex flex-wrap gap-1.5">{item.techPath.map((t) => <Badge key={t} variant="outline">{t}</Badge>)}</div></div></>}<Separator /><div><p className="text-xs text-muted-foreground">业务部门</p><div className="mt-2 flex flex-wrap gap-1.5">{item.businessFunctions.map((value) => <Badge key={value} variant="secondary">{value}</Badge>)}</div></div><Button className="w-full" asChild><Link href={item.ctaText ? "/contact" : "/assessment"}>{item.ctaText ?? "评估我的企业"}<ArrowRight /></Link></Button></CardContent></Card>
          <p className="mt-4 px-2 text-[11px] leading-5 text-muted-foreground">案例用于经验参考，不构成采购、投资或效果承诺。</p><Button variant="ghost" size="sm" asChild className="mt-2 w-full text-xs text-muted-foreground"><Link href={`/contact?type=correction&case=${item.id}`}>发现信息有误？提交更正</Link></Button>
        </aside>
      </div>
    </article>
    {related.length > 0 && <section className="border-t bg-card/45"><div className="container-page py-16"><div className="flex items-end justify-between"><div><p className="text-xs font-semibold text-primary">继续阅读</p><h2 className="mt-2 text-2xl font-semibold tracking-tight">相关案例</h2></div><Button variant="outline" asChild><Link href={`/cases?industry=${item.industry?.slug ?? "retail"}`}>更多{item.industry?.displayName ?? "行业"}案例<ArrowRight /></Link></Button></div><div className="mt-7 grid gap-4 md:grid-cols-3">{related.map((entry) => <CaseCard key={entry.id} caseStudy={entry} />)}</div></div></section>}
  </main>;
}
