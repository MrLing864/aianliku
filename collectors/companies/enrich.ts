/**
 * 企业（A 股上市公司）AI 应用案例抽取：抓取详情页 → AI 抽取 → 组装为 CaseStudy。
 * 与 government/enrich 同构，差异在 orgType=企业、sourceType=company。
 */
...
const companyCaseSchema = z.object({
  isAICase: z.boolean().describe("是否真正使用了 AI/大模型/机器学习/计算机视觉/NLP/生成式AI/知识库问答/RPA+AI 等。仅上云、建网站、普通数据库等不算。"),
  title: z.string().describe("案例标题（官方原文或精炼改写，忠于原意）"),
  summary: z.string().describe("一句话摘要，不超过100字"),
  orgName: z.string().describe("应用 AI 的企业全称，如 '宁德时代新能源科技股份有限公司'"),
  region: z.string().describe("企业总部所在城市/省份，如 '福建省宁德市' '上海市'"),
...
type CompanyCase = z.infer<typeof companyCaseSchema>;
...
function buildExtractPrompt(detailText: string, candidate: Candidate): string {
  ...
  return `你是企业 AI 应用案例分析师。请从以下企业官网发布的案例原文中提取结构化信息。
...
export async function enrichCandidate(candidate: Candidate): Promise<EnrichedResult> {
  ...
  let extracted: CompanyCase;
  try {
    const { output } = await generateText({
      model: getModel(),
      prompt: buildExtractPrompt(detailText, candidate),
      output: Output.object({ schema: companyCaseSchema }),
      providerOptions: { deepseek: { thinking: { type: "disabled" } } },
      maxOutputTokens: 6000,
      abortSignal: AbortSignal.timeout(120_000),
    });
    extracted = output as CompanyCase;
  } catch (err: any) {
  ...
  const raw = {
    ...
    companyName: extracted.orgName || candidate.company,
  ...
  const extractedForNormalize = {
    ...
    editorComment: {
      suitableFor: [extracted.region || "中国"],
      prerequisites: [],
  ...
  const cs = normalizeCase(raw, extractedForNormalize as any, extracted.orgName || candidate.company);
  cs.organization = {
    name: extracted.orgName || candidate.company,
    size: candidate.scale || "未披露",
    region: extracted.region || "中国",
    anonymous: false,
    type: "企业",
  };
