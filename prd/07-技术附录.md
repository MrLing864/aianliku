...
type SourceDocument = {
  ...
  snapshotKey?: string; // 抓取原文私有快照→EdgeOne Makers Blob 私有存储
  rightsNote?: string;
  createdAt: Date;
  updatedAt: Date;
};
...
### 18.1 当前采集器（collectors）发现策略

V1 已实现 `collectors/` 采集器，针对**政府机关**与**上市公司**两类来源采用「官网固定入口优先、泛搜兜底」策略：

- **固定入口映射**：每家政府/企业在 `collectors/government/sources.ts` 与 `collectors/companies/sources.ts` 沉淀官网固定入口（"宣传/吹牛/典型案例"栏目 URL + `list`/`detail` 类型 + `verified` 标记）。抓取时直接进这些入口，不再依赖泛搜碰运气，命中率与可维护性显著提升。
- **入口维护**：`collectors/lib/scaffold_columns.ts` 辅助脚本可扫某官网域名输出疑似案例/新闻栏目候选，人工挑选后填入 `sources.ts` 并标 `verified=true`。
- **兜底与降级**：`sources.ts` 无入口或入口全部失效时，回退 `searchCases` 泛搜（带 `site:` 限定官网域名，避免媒体软文/广告稿）。入口健康状态落盘 `/var/log/*-discover-health.json`，供人工更新映射。
- **合规**：抓取遵循 robots.txt（`canFetch`），列表页抽取详情链接用 `discoverUrls` 正则。

> 注意：`sources.ts` 是采集器的「官网入口映射代码表」，与 CloudBase 数据库集合 `sources`（去重来源元数据）**命名相同但语义不同**，后者用于案例来源去重与证据挂载。
